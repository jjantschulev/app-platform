    ...
    for (let element of block.val) {
        if (element.type === 'block') {
            ...
            if (element.val.commandType === 'ins') {
                const value = element.val.commandArgs[0];
                const [path, ...args] = value.split(" ");
                const valueAtPath = getValueAtPath(context, path);
                if (typeof valueAtPath === 'function') {
                    output += valueAtPath(...(args.map(a => a[0] === '$' ? getValueAtPath(context, a.slice(1)) : a)));
                } else {
                    output += valueAtPath;
                }
